...
import { browserPath, type DoctorInfo, scrapeDoctorInfo } from "./doctor-scraper";
...
async function getDoctorLinksWithPagination(
  ...
  try {
    ...
    try {
      ...
      const consentButton = await page.waitForSelector("#onetrust-reject-all-handler", {
        timeout: 4000
      });
      if (consentButton) {
    ...
    for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
      ...
      const urlToVisit = pageNum === 1 ? listPageUrl : `${listPageUrl}&page=${pageNum}`;
      ...
      try {
        ...
        // H3 tag inside the card, then the anchor inside that.
        const links = await page.$$eval(
          ".card-body h3 a.text-body",
          anchors => anchors.map(a => a.href).filter(href => href?.startsWith("http")) // Filter invalid links
        );
...
export async function scrapeMultipleDoctors(
  ...
  try {
    ...
    // Get all links first
    const allPages = await getDoctorLinksWithPagination(listPageUrl, maxPages, browser);
        ...
        const batchPromises = batch.map(async (url, index) => {
          try {
            ...
            if (scrapedUrls.has(url)) {
              console.log(`Skipped (already scraped): ${url}`);
              return null;
            }
